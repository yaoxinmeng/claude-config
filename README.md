...
Project-level skills and agents (`coding`, `testing`, `review`, `deps`, `coder`, `test-writer`, `code-reviewer`) are not copied; `/init-claude-configs` generates them for the project's own stack (see below).
...
Seven files under `<project>/.claude/`, each written for the project's language, framework, and toolchain. A rule that would be true in any repo is left to the global skills; these hold only what is specific to this one, each convention with the file that shows it.
...
| `agents/code-reviewer.md` | Agent tool, opus | A copy of the global `code-reviewer` that also preloads `coding` and `testing`. A project-level agent overrides the user-level one with the same name, so inside the project the reviewer judges idiom by the stack instead of guessing it from the diff. Re-run the skill after changing the global agent. |

On a greenfield repo the checks table is marked unverified until the first coding session has run every row.

`docs-researcher` lives in `home/`, so a project on a machine without the global config does not have it; the generated `deps` and the `aws-cdk` skill say to do the lookup by hand and report that they did, rather than answering from memory. The generated `review` and `code-reviewer` depend on `code-reviewer`, `security-reviewer`, and `simplifier` from the same place.
...
4. Run `/init-claude-configs`. It reads `docs/spec/stack.md`, asks about whatever the spec leaves open, and writes the `coding`, `testing`, `review`, and `deps` skills and the `coder`, `test-writer`, and `code-reviewer` agents for that stack.
